...
void nic_stats_display(portid_t port_id)
{
    ...
    capture_info.tot_pkts = stats.ipackets;
    capture_info.tot_bytes = stats.ibytes;
    capture_info.recv_bytes = stats.ibytes + (capture_info.tot_pkts * 24);
    capture_info.sent_bytes = stats.obytes + (stats.opackets * 24);
    capture_info.recv_pkts = stats.ipackets;
    capture_info.sent_pkts = stats.opackets;
    capture_info.drop_pkts = stats.imissed + stats.ierrors;
    capture_info.rx_thoughput = mbps_rx;
    capture_info.tx_thoughput = mbps_tx;
}
...
void rx_queue_infos_display(portid_t port_id, uint16_t queue_id)
{
...
void tx_queue_infos_display(portid_t port_id, uint16_t queue_id)
{
    struct rte_eth_burst_mode mode;
...
void port_reg_bit_display(portid_t port_id, uint32_t reg_off, uint8_t bit_x)
{
    uint32_t reg_v;
...
void port_flow_tunnel_create(portid_t port_id, const struct tunnel_ops *ops)
{
    ...
    if (!flt)
    {
        ...
        flt->id = LIST_EMPTY(&port->flow_tunnel_list) ? 1 : LIST_FIRST(&port->flow_tunnel_list)->id + 1;
              ...
              struct rte_flow_error *error)
{
    ...
    if (!pf)
    {
        rte_flow_error_set(error, errno, RTE_FLOW_ERROR_TYPE_UNSPECIFIED, NULL,
                           "calloc() failed");
        return NULL;
    }
...
age_action_get(const struct rte_flow_action *actions)
{
    for (; actions->type != RTE_FLOW_ACTION_TYPE_END; actions++)
    {
        ...
        {
        case RTE_FLOW_ACTION_TYPE_AGE:
            return (struct rte_flow_action_age *)(uintptr_t)actions->conf;
        default:
            break;
        }
    }
...
/** Dump flow rules. */
int port_flow_dump(portid_t port_id, bool dump_all, uint32_t rule_id,
...
get_rx_ring_size(portid_t port_id, queueid_t rxq_id, uint16_t *ring_size)
{
	 ...
	 * ring_size stored in goblin will be used for validity verification.
...
void port_rss_hash_key_update(portid_t port_id, char rss_type[], uint8_t *hash_key,
...
setup_fwd_config_of_each_lcore(struct fwd_config *cfg)
{
    ...
    // rte_exit(EXIT_FAILURE, "debuging setup_fwd_config_of_each_lcore>>>>>> nb_fs_per_lcore: %u %u %u\n", nb_fs_per_lcore, nb_fs, nb_fc);
}
...
simple_fwd_config_setup(void)
{
    ...
    cur_fwd_config.nb_fwd_streams = (streamid_t)cur_fwd_config.nb_fwd_ports;
    ...
    {
        cur_fwd_config.nb_fwd_lcores = (lcoreid_t)cur_fwd_config.nb_fwd_ports;
    }
...
dcb_fwd_config_setup(void)
{
    ...
    for (lc_id = 0; lc_id < cur_fwd_config.nb_fwd_lcores; lc_id++)
    {
        ...
        if (numa_support && (nb_fwd_ports <= (nb_ports >> 1)))
            rxp = (portid_t)(rxp + ((nb_ports >> 1) / nb_fwd_ports));
        else
            rxp++;
        if (rxp >= nb_fwd_ports)
            return;
...
icmp_echo_config_setup(void)
{
    ...
    if ((nb_txq * nb_fwd_ports) < nb_fwd_lcores)
        cur_fwd_config.nb_fwd_lcores = (lcoreid_t)(nb_txq * nb_fwd_ports);
    else
        cur_fwd_config.nb_fwd_lcores = (lcoreid_t)nb_fwd_lcores;
    cur_fwd_config.nb_fwd_ports = nb_fwd_ports;
    cur_fwd_config.nb_fwd_streams =
...
again:
    ...
    // rte_exit(EXIT_FAILURE, "set_fwd_lcores_list %u\n", nb_fwd_lcores);
    return 0;
}
...
void parse_fwd_portlist(const char *portlist)
{
...
int parse_fec_mode(const char *name, uint32_t *mode)
{
    uint8_t i;
...
void show_fec_capability(unsigned int num, struct rte_eth_fec_capa *speed_fec_capa)
{
    unsigned int i, j;
...
void set_pkt_forwarding_mode(const char *fwd_mode_name)
{
    struct fwd_engine *fwd_eng;
    unsigned i;
    printf("set_pkt_forwarding_mode %s\n", fwd_mode_name);
    i = 0;
    while ((fwd_eng = fwd_engines[i]) != NULL)
    {
...
void add_rx_dump_callbacks(portid_t portid)
{
    ...
    if (ret != 0)
        return;
    // printf("add_rx_dump_callbacks >>>> nb_rx_queues>>>>%u\n", dev_info.nb_rx_queues);
    // rte_exit(EXIT_FAILURE, "Debuging %d\n", dev_info.nb_rx_queues);
    for (queue = 0; queue < dev_info.nb_rx_queues; queue++)
    {
        if (!ports[portid].rx_dump_cb[queue])
        {
            if (cur_fwd_config.fwd_eng->rx_callback != NULL)
            {
                ports[portid].rx_dump_cb[queue] = rte_eth_add_rx_callback(portid, queue, cur_fwd_config.fwd_eng->rx_callback, NULL);
            }
            else
            {

                ports[portid].rx_dump_cb[queue] = rte_eth_add_rx_callback(portid, queue, dump_rx_pkts, NULL);
            }
        }
    }
}
...
void vlan_tpid_set(portid_t port_id, enum rte_vlan_type vlan_type, uint16_t tp_id)
{
    int diag;
