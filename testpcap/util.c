    ...
    for (i = 0; i < nb_pkts; i++)
    {
        ...
        if (!ret)
        {
            ...
            if (info.flags & RTE_FLOW_RESTORE_INFO_TUNNEL)
            {
                ...
                port_tunnel = port_flow_locate_tunnel(port_id, &info.tunnel);
                MKDUMPSTR(print_buf, buf_size, cur_len,
                          " - tunnel");
                if (port_tunnel)
                    MKDUMPSTR(print_buf, buf_size, cur_len,
                    ...
                    MKDUMPSTR(print_buf, buf_size, cur_len,
                              " %s", "-none-");
                MKDUMPSTR(print_buf, buf_size, cur_len,
                          " type %s", port_flow_tunnel_type(&info.tunnel));
            }
            else
            {
                MKDUMPSTR(print_buf, buf_size, cur_len,
...
void eth_set_promisc_mode(uint16_t port, int enable)
{
    int ret;
...
int eth_macaddr_get_print_err(uint16_t port_id, struct rte_ether_addr *mac_addr)
{
    int ret;
