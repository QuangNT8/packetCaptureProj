...
uint8_t enable_l7 = 0;      /**< L7 protocol. */
int goblin_logtype;         /**< Log type for goblin logs */
...
uint16_t mbuf_data_size[MAX_SEGS_BUFFER_SPLIT] = {
    DEFAULT_MBUF_DATA_SIZE};        /**< Mbuf data space size. */
uint32_t param_total_num_mbufs = 0; /**< number of mbufs in all pools - if
                                      * specified on command-line. */
uint16_t stats_period = 0;          /**< Period to show statistics (disabled by default) */
                 ...
                 unsigned int socket_id, uint16_t size_idx)
{
    ...
    case MP_ALLOC_XBUF:
    {
        ...
        TESTPMD_LOG(INFO, "preferred mempool ops selected: %s\n",
                    rte_mbuf_best_mempool_ops());
        rte_mp = rte_pktmbuf_pool_create_extbuf(pool_name, nb_mbuf, mb_mempool_cache,
...
init_config(void)
{
    ...
    fwd_lcores = rte_zmalloc("goblin: fwd_lcores",
    ...
    {
        fwd_lcores[lc_id] = rte_zmalloc("goblin: struct fwd_lcore",
    ...
    else
    {
        ...
        for (i = 0; i < mbuf_data_size_n; i++)
            mempools[i] = mbuf_pool_create(mbuf_data_size[i],
                                           nb_mbuf_per_pool,
                                           socket_num == UMA_NO_CONFIG ? 0 : socket_num, i);
    }
...
int init_fwd_streams(void)
{
    ...
    if (nb_fwd_streams)
    {
        fwd_streams = rte_zmalloc("goblin: fwd_streams",
        ...
        for (sm_id = 0; sm_id < nb_fwd_streams; sm_id++)
        {
            fwd_streams[sm_id] = rte_zmalloc("goblin:"
                                             " struct fwd_stream",
    ...
    for (j = 0; j < 2; j++)
    {
        for (rxp = 0; rxp < cur_fwd_config.nb_fwd_ports; rxp++)
        {
            for (rxq = 0; rxq < nb_rxq; rxq++)
            {
                ...
                /**
				* goblin can stuck in the below do while loop
...
/* ************************************ */
static void app_uinit(portid_t nb_p)
{
    printf("app_uinit nb_p %u\n", nb_p);
    appuninitial_t p_appuninit;
    p_appuninit = cur_fwd_config.fwd_eng->app_uninit;
...
{
    printf("app_init nb_p %u\n", nb_p);
    appinitial_t p_appinit;
    p_appinit = cur_fwd_config.fwd_eng->app_init;
...
run_pkt_fwd_on_lcore(struct fwd_lcore *fc, packet_fwd_t pkt_fwd)
{
    ...
    do
    {
        ...
        {
            (*pkt_fwd)(fsm[sm_id]); // call user services;
        }
...
{
    struct fwd_lcore *arg = fwd_arg;
    // printf("start_pkt_forward_on_core >>>>>>> cpuid_idx %d\n", arg->stream_nb);
    run_pkt_fwd_on_lcore(arg, cur_fwd_config.fwd_eng->packet_fwd);
    return 0;
}
...
launch_packet_forwarding(lcore_function_t *pkt_fwd_on_lcore)
{
    ...
    // rte_exit(EXIT_FAILURE, "debuging %u\n", 0);
...
void start_packet_forwarding(int with_tx_first)
{
    ...
    // rte_eal_mp_remote_launch(start_pkt_forward_on_core, NULL, CALL_MAIN);
}
...
int rx_queue_setup(uint16_t port_id, uint16_t rx_queue_id,
                   ...
                   struct rte_eth_rxconf *rx_conf, struct rte_mempool *mp)
{
    ...
    for (i = 0; i < rx_pkt_nb_segs; i++)
    {
        ...
        rx_seg->length = rx_pkt_seg_lengths[i] ? rx_pkt_seg_lengths[i] : mbuf_data_size[mp_n];
        rx_seg->offset = i < rx_pkt_nb_offs ? rx_pkt_seg_offsets[i] : 0;
            ...
            for (qi = 0; qi < nb_rxq; qi++)
            {
                    ...
                    (rxring_numa[pi] != NUMA_NO_CONFIG))
                {
                    struct rte_mempool *mp =
                        mbuf_pool_find(rxring_numa[pi], 0);
                    if (mp == NULL)
                    {
                ...
                {
                    struct rte_mempool *mp =
                        mbuf_pool_find(port->socket_id, 0);
                    if (mp == NULL)
                    {
                        printf("Failed to setup RX queue:"
                               "No mempool allocation"
...
{
    printf("pmd_test_exit\n");
    portid_t pt_id, nb_p = 0;
    unsigned int i;
    int ret;
                 ...
                 uint8_t pfc_en)
{
    ...
    if (dcb_mode == DCB_VT_ENABLED)
    {
        ...
        eth_conf->rxmode.mq_mode =
            (enum rte_eth_rx_mq_mode)(rx_mq_mode & ETH_MQ_RX_VMDQ_DCB);
    ...
    else
    {
        ...
        eth_conf->rxmode.mq_mode =
            (enum rte_eth_rx_mq_mode)(rx_mq_mode & ETH_MQ_RX_DCB_RSS);
...
init_port(void)
{
    ...
    ports = rte_zmalloc("goblin: ports",
...
print_stats(void)
{
    port_print_stats_t p_print_stats;
    uint8_t i;
    portid_t pt_id;
    const char clr[] = {27, '[', '2', 'J', '\0'};
    const char top_left[] = {27, '[', '1', ';', '1', 'H', '\0'};
    p_print_stats = cur_fwd_config.fwd_eng->print_stats;
    /* Clear screen and move to top left */
    if (p_print_stats == NULL)
    {
        // printf("%s%s", clr, top_left);
        printf("\n  ======================= Port statistics From Main ========================\n");
    }
    else
    {
        printf("\n  ===================== Port statistics From User App =======================\n");
    }
    for (i = 0; i < cur_fwd_config.nb_fwd_ports; i++)
    {
...
{
    if ((signum == SIGINT || signum == SIGTERM) && (f_quit == 0))
    {
...
/* ************************************ */
char ifaces[RTE_MAX_ETHPORTS][64];

int add_interfaces()
{
    int ret = 0;

    // strcpy(ifaces[0], "enp7s0");
    // strcpy(ifaces[1], "wlp8s0");
    ...
    if (ret > 0)
    {
        for (i = 0; i < max_iface_index; i++)
        {
            ...
            snprintf(vdev_name, sizeof(vdev_name), "net_pcap%u", i);
            snprintf(vdev_args, sizeof(vdev_args), "rx_iface=%s,tx_pcap=%s.pcap", ifaces[i], ifaces[i]);
            // snprintf(vdev_args, sizeof(vdev_args), "tx_pcap=%s.pcap", ifaces[i]);
            printf("iface: %u %s %u %s\n", i, ifaces[i], max_iface_index, vdev_args);
            printf("=========================================\n");
...
int main(int argc, char **argv)
{
    ...
    goblin_logtype = rte_log_register("goblin");

    if (goblin_logtype < 0)
        rte_exit(EXIT_FAILURE, "Cannot register log type");

    rte_log_set_level(goblin_logtype, RTE_LOG_DEBUG);
    ...
    if (nb_ports == 0)
        rte_exit(EXIT_FAILURE, "No probed ethernet devices\n");
    // rte_exit(EXIT_FAILURE, "debuging >>>>>> nb_ports: %u\n", nb_ports);

    /* allocate port structures, and init them */
    init_port();

    set_def_fwd_config();
    // rte_exit(EXIT_FAILURE, "debuging >>>>>> nb_cfg_lcores: %u\n", nb_cfg_lcores);
    if (nb_lcores == 0)
    {
    ...
    if (nb_ports > 0)
    {
        ...
        {
            rte_eth_dev_get_name_by_port(i, tmp);
            attach_port(tmp);
            // setup_attached_port(i);
            printf("Port Name : >>>>> %s\n", tmp);
        }

        set_fwd_lcores_number(nb_ports);
        // verbose_level = 0;
        // set_pkt_forwarding_mode("io");
        set_pkt_forwarding_mode("goblin");
    }
