...
typedef void (*port_print_stats_t)(portid_t pi);
typedef void (*appinitial_t)(portid_t nb_p);
typedef void (*appuninitial_t)(portid_t nb_p);

struct fwd_engine
{
    const char *fwd_mode_name; /**< Forwarding mode name. */
    appinitial_t app_init;
    ...
    port_print_stats_t print_stats;
    rte_rx_callback_fn rx_callback;
    appuninitial_t app_uninit;
};
...
extern struct fwd_engine goblin_engine;
...
extern uint8_t enable_l7;          /**< Ebables L7 Protocol. */
extern int goblin_logtype;         /**< Log type for goblin logs */
...
extern char ifaces[RTE_MAX_ETHPORTS][64];
...
extern struct capture_interfaces_info capture_info;
...
void parse_fwd_portlist(const char *port);
...
#define TESTPMD_LOG(level, fmt, args...) \
    rte_log(RTE_LOG_##level, goblin_logtype, "goblin: " fmt, ##args)
