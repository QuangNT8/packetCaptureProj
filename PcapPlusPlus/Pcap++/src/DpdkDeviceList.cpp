...
namespace pcpp
{
    ...
    bool DpdkDeviceList::m_IsPcapgen = false;
    std::vector<std::string>
        DpdkDeviceList::m_ifaces;
    ...
    bool DpdkDeviceList::initDpdk(CoreMask coreMask, uint32_t mBufPoolSizePerDevice, uint8_t masterCore)
    {
        ...
        for (int i; i < m_ifaces.size(); i++)
        {
            ...
            if (m_IsPcapgen)
            {
                snprintf(vdev_args, sizeof(vdev_args), "rx_iface=%s,tx_pcap=%s.pcap", m_ifaces[i].c_str(), m_ifaces[i].c_str());
            }
            else
            {
                snprintf(vdev_args, sizeof(vdev_args), "rx_iface=%s", m_ifaces[i].c_str());
            }
    ...
    bool DpdkDeviceList::startDpdkWorkerThreads(CoreMask coreMask, std::vector<DpdkWorkerThread *> &workerThreadsVec)
    {
        ...
        {
            LOG_ERROR("Number of cores in core mask different from workerThreadsVec size %u,%u", numOfCoresInMask, workerThreadsVec.size());
            return false;
        }
    ...
    int DpdkDeviceList::addVirtualDev(std::vector<std::string> ifaces, bool pcapgen)
    {
        m_ifaces = ifaces;
        m_IsPcapgen = pcapgen;
